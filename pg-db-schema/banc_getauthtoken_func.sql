...
BEGIN
   ...
   IF recfound = 1 THEN 
   	  auth := true;
		  SELECT personid, primeid, priv INTO persnid, primid, authlevel FROM banc.creds WHERE uid=userid and email=mail and md5Passwd=pwdmd5;
...
-- Insert a new sessiontoken for uid  OR
    -- Update the new session token for uid and email, if record is present   Handle if duplicate as an exception
		  INSERT INTO banc.sessiontoken (uid, email, primeid, personid, token, authlevel)
			    VALUES (userid, mail, primid, persnid, stoken, authlevel) ;
