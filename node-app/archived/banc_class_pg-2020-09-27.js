...
class setupDataHelper {
    ...
    constructor(verbose) {
        ...
        this.apiAuth = JSON.parse(fs.readFileSync("apiAuthorizationSetup.json", "utf8"));
        this.clientApiKeys = JSON.parse(fs.readFileSync("apiClientKeys.json", "utf8"));
    ...
    updateUnprocessedDataTable(procData, banc_transaction, settlement, aCallback) {
    ...
    processPayData(rawData, primeid2Use, aCallback) {
        ...
        var processSummaryTransaction = this.processSummaryTransaction.bind(this);
        // create function data
        var data = this.sortAndBuildTransactionData(form_ref_id, form_memo, form_date, raw_data);
        /* var primeid = 35;
        outRes.msg = data;
        aCallback(outRes);
        return;
        */
        ...
        function procPayFnCB(output) {
            ...
            if (flow_step == 0) {
                ...
                if (data.membership.length > 0) {
                    console.log("Membership Data: ", data.membership[0]);
                    processMembershipTransaction(primeid, data.membership[0], flow_step, procPayFnCB);
                    return;
                }
            }
            //
            ...
            if (flow_step == 2) {
                ...
                if (data.summary.length > 0) {
                    console.log("Summary Transaction Data: ", data.summary);
                    processSummaryTransaction(primeid, data.summary[0], flow_step, procPayFnCB);
                    return;
                }
            }
            if (flow_step == 3) {
                outRes.summary = output;
                outRes.msg = "Raw Pay Data processed for form_ref_id=" + outRes.form_ref_id;
                outRes.err = false;
                aCallback(outRes);
                return;
            }
    ...
    verifyApi2Execute(api, authgroup) {
        if (api in this.api2AuthGroupMap) {
            return (this.api2AuthGroupMap[api].includes(authgroup));
        }
        return false;
    }
    //
    verifyClientIdAccess(api, clientId, apiKey) {
        var client;
        //   console.log(api, clientId, apiKey, this.clientApiKeys);
        if (clientId in this.clientApiKeys) {
            client = this.clientApiKeys[clientId];
            //     console.log(client);
            if (apiKey == client.apikey) {
                ...
                if (api in this.api2AuthGroupMap) {
                    ...
                    return (this.api2AuthGroupMap[api].includes(group));
                }
            }
        }
        return false;
    ...
    */
    ...
    //
    setEventInfo() {
        const sqlStmt = "SELECT eventname, venue, entity_id, event_year, showlink, start_date, end_date FROM banc.event;";
        //
    ...
    isTokenValid(userid, email, token, actionCallback) {
        ...
        var sqlStmt = "SELECT primeid, personid, authlevel FROM banc.sessionToken WHERE uid=$1 and email=$2 and token=$3 " + expiryTime;
    ...
    setTransactionTypeInfo() {
        const sqlStmt = "SELECT category, subcategory, line_item, type_id, mapping_string, adult_count, child_count FROM banc.transaction_type;";
        this.pool
            .connect()
            .then(client => {
                return client
    ...
    sortAndBuildTransactionData(form_ref_id, form_memo, form_date, jData) {
        ...
        */
        ...
        for (let i = 0; i < allTxns.length; i++) {
            ...
            var adult_count = this.getTransactionInfoFromId(out.transaction_type_id).adult_count;
            var child_count = this.getTransactionInfoFromId(out.transaction_type_id).child_count;
            ...
            if (txndata.category == "BANC") {
                if ("membership" in jData) {
                    out.name = "Membership/" + txndata.category + "/" + txndata.sub_category + "/" + txndata.line_item;
                    out.info = jData.membership.type;
                    out.year = jData.membership.year;
                    out.form_memo = form_memo + "/" + txndata.memo + "/" + out.info + "/" + out.year;
                    txns.membership.push(out);
                    smry.name = smry.name + txndata.category + "[Membership - " + out.year + "] " + txndata.line_item;
                    smry.year = out.year;
                }
                ...
                if ("event" in jData) {
                    ...
                    out.info = "";
                    out.name = out.eventname + "/" + txndata.category + "/" + txndata.sub_category + "/" + txndata.line_item;
                    out.year = jData.event.year;
                    out.form_memo = out.eventname + ", " + out.year + ":" + txndata.line_item + "[" + form_memo + "/" + txndata.memo + "]";
                    smry.year = out.year;
                    let lineitem = txndata.line_item;
                    if (!smryEventText) {
                        smryEventText = "[" + out.eventname + ", " + out.year + "] " + lineitem;
                    } else {
                        smryEventText = smryEventText + ", " + lineitem;
                    }
                    //           console.log(lineitem);
                    //if ((new RegExp("/Family|Student|Senior/")).exec(lineitem) !== null) {
                    out.info = this.assocLinkInfo["member"].display_name + "/" + out.eventname;
        ...
        if (jData.paypal.length > 0) {
            for (let i = 0; i < jData.paypal.length; i++) {
                ...
                out.transaction_type_id = this.getTransactionId("paypal", null, paypalLine.type);
        ...
        if (jData.cash.length > 0) {
            smry.transaction_type_id = this.getTransactionId("cash", "receipt", "Received");
            ...
            if ("event" in jData || "membership" in jData) {
                smry.transaction_type_id = this.getTransactionId("paypal", "receipt", "Website");
                smry.name = smry.name + smryEventText;
                smry.form_memo = smry.name;
                txns.summary.push(smry);
                smry.summary = true;
    ...
    */
    setTransactionWithPrime(jData, primeid = null, info = null, key = null, aCallback) {
        ...
        if (key > 0) {
            sqlStmt = "UPDATE " + entitytable + " SET amount=" + jData.amount + "," +
                ...
                "', status='" + jData.status + "' WHERE entity_id=" + key + " RETURNING *;"
    ...
    executeAddressSQL(jAdr, primeid, key = null, aCallback) {
        ...
        console.log(jAdr);
        if (key == null || key <= 0) {
            sqlStmt = `INSERT INTO ${tableAddress} (street, address2, city, state, zip, country) VALUES (` +
                `'${jAdr.street}', '${jAdr.address2}', '${jAdr.city}','${jAdr.state}','${jAdr.zip}','${jAdr.country}') ` +
                'ON CONFLICT (street, city, zip) DO NOTHING RETURNING *;';

            if (primeid > 0) {
                sql2 = "INSERT INTO " + tableAssoc + " (entity1_id, entity1, entity2_id, entity2, " +
            ...
            sqlStmt = `UPDATE ${tableAddress} SET street=${jAdr.street}, address2=${jAdr.address2}, ` +
                `city=${jAdr.city}, state=${jAdr.state}, zip=${jAdr.zip}, country=${jAdr.country} WHERE entit_id=${key}; `;
            sql2 = null;
            cond = false;
        }
        console.log(sqlStmt);
        this.execute2SqlTxn(this.setDbCallData("Execute Address with prime", sqlStmt, null, sql2, null, cond), addressCB);
        return;
        //
    ...
    executeCommSQL(jFC, primeid, key = null, aCallback) {
        ...
        if (key === null || key <= 0) {
            ...
            sqlStmt = `INSERT INTO ${tablePC} (telephone, mobile, email) VALUES (` +
            ...
            sqlStmt = ` "UPDATE ${tablePC} SET email=${jFC.email}, telephone=${jFC.telephone}, ` +
                `mobile=${jFC.mobile} WHERE entit_id=${key}; `;
            sql2 = null;
            cond = false;
        }
        console.log(sqlStmt);
        this.execute2SqlTxn(this.setDbCallData("Execute Communication with prime", sqlStmt, null, sql2, null, cond), pcommCB);
        return;
        //
    ...
    */
    executePersonSQL(jData, primeid, prime, dep, isMinor, key = null, aCallback) {
        ...
        */
         ...
         */
        // do the test
        var test4email = `SELECT * from banc.person where email='${jData.email}';`;
        //console.log(test4email);
        var execute2SqlTxn = this.execute2SqlTxn.bind(this);
        var setDbCallData = this.setDbCallData.bind(this);
        this.execute1SqlsWithCommit(this.setDbCallData("Execute Person SQL", test4email, null, null, null, false), personCB);
        //
        function personCB(output) {
            ...
            if (flow_step == 0) {
                ...
                if (result.rowCount > 0) {
                    ...
                    if ((row.firstname).toLowerCase() != (jData.firstname).toLowerCase() ||
                        (row.middlename).toLowerCase() != (jData.middlename).toLowerCase() ||
                        (row.lastname).toLowerCase() != (jData.lastname).toLowerCase()) {
                        let initials = row.firstname.charAt(0) + ". <middlename> " + row.lastname.charAt(0) + ".";
                        outRes.err = true;
                        outRes.person = jData;
                        // outRes.sql2_result = row;
                        outRes.err_msg = "ERROR - Email [" + row.email + "] already exists for initials " + initials;
                        aCallback(outRes);
                        return;
                    }
                }
                execute2SqlTxn(setDbCallData("Execute Person SQL ", sqlStmt, null, sql2, null, cond), personCB);
                return;
            }
            if (flow_step == 1) {
    ...
    setAssocLink(jData, key = null, aCallback) {
        ...
        cond = false;
        args = [jData.entity1_id, jData.entity2_id, jData.entity1, jData.entity2, jData.name, jData.link_type_id,
        jData.additional_info, jData.adult_count, jData.child_count, jData.guest_count];
        //
        if (key == null || key <= 0) {
            ...
            sqlStmt = "UPDATE " + this.config.linkTables['association'] +
        ...
        this.execute1SqlsWithCommit(this.setDbCallData("Set Assoc Link", sqlStmt, args, null, null, cond), aCallback);
        return;
    ...
    associatedPersons(id, aCallback) {
        ...
        console.log(sqlStmt);
        this.execute1SqlsWithCommit(this.setDbCallData("associatedPerson", sqlStmt, null, null, null, false), assocPersonsCB);
        return;
        //
...
//
    ...
    associatedAddress(id, aCallback) {
        ...
        console.log(sqlStmt);
        this.execute1SqlsWithCommit(this.setDbCallData("associatedAddress", sqlStmt, null, null, null, false), aCallback);
        return;
    }
    /* *************** */
    associatedFamilyComm(id, aCallback) {
        ...
        console.log(sqlStmt);
        this.execute1SqlsWithCommit(this.setDbCallData("associatedPerson", sqlStmt, null, null, null, false), aCallback);
        return;
    }
    /**
    ...
    getMembershipPaid(primeid, aCallback) {
        ...
        " from banc.community_link c, banc.transaction t, banc.transaction_link l " +
    ...
    getRegisteredEvents(primeid, year, aCallback) {
        ...
        if (year > 0) {
            sqlStmt = "SELECT e.eventname, e.event_year, al.name as registration, al.adult_count, al.guest_count, " +
                " al.child_count, al.update_date as date, al.link_id from banc.event e, banc.association_link al " +
    ...
    getTransactions(primeid, dateFrom, dateTo, all_txn, aCallback) {
        ...
        if (dateFrom != null && dateTo != null) {
            sqlStmt = "Select tl.name as transaction_name, t.amount, t.service_fee_paid as fee_paid, t.status, " +
                ...
                "from banc.transaction t, banc.transaction_link tl, banc.transaction_type tt where " +
    ...
    getBANCMembershipPaid(primeid, yearFrom, yearTo, all_members, aCallback) {
        ...
        if (yearFrom != null && yearTo != null) {
            sqlStmt = "Select o.orgname, c.affiliation, cl.year as membership_year, c.ismember, cl.renewal_date, " +
                "c.entity_id as banc_community_id, cl.link_id " +
                "from banc.banc_community c, banc.community_link cl, banc.organization o  where " +
    ...
    processMembershipTransaction(primeid, jMship, flowStep, anExtCallback) {
        ...
        var sqlStmt = "INSERT INTO " + tableAssoc + " (entity1_id, entity1, entity2_id, entity2, name, affiliation_id, year, renewal_date) " +
        ...
        function orchMShipCB(output) {
            ...
            if (flow_step == 0) {
                ...
                flow_step = 2;
                bancTxnLinkSql = "INSERT INTO " + linktable1 + " (entity1_id, entity2_id, entity1, entity2, name, additional_info, " +
                    "form_ref, txn_form_date) VALUES " +
                ...
                //
                execute1SqlsWithCommit(setDbCallData("Processs Membership - banc_txn_link", bancTxnLinkSql, null, null, null, true), orchMShipCB);
                return;
                //
            } else if (flow_step == 2) {
    ...
    processEventTransaction(primeid, jTxns, flowStep, anExtCallback) {
        ...
        function orchEventCB(output) {
            ...
            if (flow_step == 0) {
                ...
                return;
            } else if (flow_step == 1) {  // process the event to this transaction link
                ...
                eventTxnLinkSql = "INSERT INTO " + linktable1 + " (entity1_id, entity2_id, entity1, entity2, name, additional_info, " +
                    "form_ref, txn_form_date) VALUES " +
                    `(${event_entity_id},${txnEntityId}, '${entity2}', 'transaction','${jTxn.name}','${jTxn.info}','${jTxn.form_ref}','${jTxn.form_date}') RETURNING *;`;

                execute1SqlsWithCommit(setDbCallData("Processs Event - event_txn_link", eventTxnLinkSql, null, null, null, true), orchEventCB);
                return;
                //
            } else if (flow_step == 2) {  // Is there another event transaction to process, if not exit.
    ...
    */
    processSummaryTransaction(primeid, jSmry, flowStep, anExtCallback) {
        ...
        function orchSumCB(output) {
            ...
            if (output.sql2_result) sql2_result = JSON.parse(output.sql2_result);
            //
            console.log("orchSumCB -- flow_step: ", flow_step, "sql1: ", sql1_result, "sql2: ", sql2_result);
            // process steps
    ...
    ******** */
    ...
    getPersonWithPrime(firstname, middlename, lastname, email, primeid, personid, aCallback) {
        ...
        if (primeid > 0 && personid > 0) {
            ...
            sql1 = (mnameNotPresent) ? sql1 + "and middlename='' " : sql1 + "and middlename= '" + middlename + "' ";
    ...
    * 
    */
    processPersonsInfo(primeInfo, spouseInfo, depInfo, address, primcomm, loginInfo, aCallback) {
        // bind the functions to this
        var execute1SqlsWithCommit = this.execute1SqlsWithCommit.bind(this);
        var setDbCallData = this.setDbCallData.bind(this);
        var getUTCTimestamp = this.getUTCTimestamp.bind(this);
        ...
        function procpersonsCB(output) {
            ...
            if (flow_step == 0) {
                ...
                if (update) { // update prime if present
                    if (primeInfo != null) {
                        ...
                        executePersonSQL(primeInfo, primeid, prime, dep, isMinor, key, procpersonsCB);
                        return;
                    }
                    flow_step = 1;
                } else {  // insert new person (prime)
                    let key = null;
                    let prime = 1;
                    let dep = 0;
                    let isMinor = 0;
            ...
            if (flow_step == 1) {
                ...
                if (spouseInfo != null) {
                    ...
                    if (update) { // update spouse if present
                        ...
                        outRes.flow_step = 2;
                        let primeid = primeInfo.entity_id;
                        executePersonSQL(spouseInfo, primeid, prime, dep, isMinor, key, procpersonsCB);
                        return;
                    } else {  // insert if data is present
                        let key = null;
                        let prime = 0;
                        let dep = 0;
                        let isMinor = 0;
            ...
            if (flow_step == 2) {
                ...
                if (address != null) {
                    ...
                    if (update) { // update address if present
            ...
            if (flow_step == 3) {
                ...
                if (primcomm != null) {
                    ...
                    if (update) { // update primaryCommunication if present
            ...
            if (flow_step == 5) {
                ...
                if (depsLen > 0 && i < depsLen) {
                    ...
                    if (update) { // update dep 
                        ...
                        outRes.sub_step = i + 1;
                        let primeid = primeInfo.entity_id;
                        executePersonSQL(aDep, primeid, prime, dep, isMinor, key, procpersonsCB);
                        return;
                    } else {  // insert if data is present
                        let aDep = depInfo[i];
                        let key = null;
 ...
 */
    ...
    */
    ...
    personData(person_json, update = null) {
        ...
        if (!test.valid) return false;
        if (!(person_json.middlename === null || typeof person_json.middlename === 'undefined')) {
            if (person_json.middlename.length > 0) {
