...
class setupDataHelper {
    ...
    setDbCallData(txn_name,sql1, value1, sql2, value2, isAssoc) {
        var data = {
            "name": txn_name,
            "sql1": sql1,
            "args1": value1,
    ...
    */
    ...
    //
    // test credentials before login
    testCreds(uid, pwd, mail, cell) {
        return true, "OK";
    }
    //
    // test credentials before login
    testTokens(uid, pwd, mail, cell) {
        return true, "OK";
    }
    //
    printData() {
...
class TransactionManager {
    ...
    constructor(config, beginCallback) {
        this.config = config;
    }
    //
    //
    executeTransactions(actionCallback) {
        //
        ...
        pool.connect((err, client, done) => {
            ...
            client.query('BEGIN', (err, res) => {
                callQuery(err, res, actionCallback, step)
            });
        });

        function callQuery(err, res, cb, step) {
            if (shouldAbort(err)) return
            const out = cb(res, err, step);
            const qText = out.sql;
            const qValues = out.values;
            const qDone = out.done;
            if (!qDone) {
                var step = out.step + 1;
