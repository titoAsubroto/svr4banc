...
const field_type_json = {
    "email": {
        "pattern": "/^([a-zA-Z0-9_\-\.]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([a-zA-Z0-9\-]+\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\]?)$/gm",
        "len": -1,
        "msg": "Email address pattern failed!"
    },
    "uid": {
        "pattern": "/^(?!^[0-9]*$)(?!^[a-zA-Z]*$)^([a-zA-Z0-9]{6,20})$/gm",
        "len": 20,
        "msg": " User id pattern failed or User id max. length failed: "
    },
    "md5": {
...
*/
    ...
    */
    validData(field, field_type, field_len_limit = null) {
        const field_str = field.toString();
        const field_len = field.length();
        var msg = "";
        if (field_type in field_type_json) {
            const ft = field_type_json.field_type;
            const ft_len = ft.len_limit > field_len_limit ? ft.len_limit : field_len_limit;
            const regex = new Regex(ft.pattern);
        } else {
            msg = "Field_type " + field_type + " not found / unknown!"
            return false, msg;
        }
        var msg = "";
        if (field_len > ft_len) {
            msg = "Error: Field length more than limit " + field_len_limit.toString();
            return false, msg;
        }
...
/* create simple Pg client */
class aSimplePgClient {
    ...
    constructor(config, verbose) {
        ...
        /*
                this.conn = new Client({
                    ...
                    user: this.config.Pg_uid,
                    password: this.config.Pg_pwd,
    ...
    callQuery(sql, args) {
        return new Promise((resolve, reject) => {
            this.conn.query(sql, args, (err, rows) => {
                console.log('---- Called using SQL Statement -----');
                if (err)
                    return reject(err);
                resolve(rows);
            });
    ...
    /*
    * Support a prepared query
    * name <-   // give the query a unique name
    * const qry = {  name: 'fetch-user', 
        text: 'SELECT * FROM user WHERE id = $1',values: [1]}
    */
    callQueryPrep(qry, args) {
        return new Promise((resolve, reject) => {
            this.conn.query(qry, (err, res) => {
                console.log('---- Called using Prepared Statement -----');
                if (err) {
                    return reject(err);
                }
                resolve(res);
...
exports.aSimplePgClient = aSimplePgClient;