...
class setupDataHelper {
    ...
    setEventInfo2() {
        ...
        conn.connect(function (err) {
            ...
            if (err) throw err;
            const sqlStmt = "SELECT eventname, entity_id, event_year FROM event;";
            conn.query(sqlStmt, function (err, result, fields) {
                // if any error while executing above query, throw error
                if (err) throw err;
    ...
    sqlPerson(jData, prime, dep, isMinor, key = null) {
        ...
        if (key <= 0 || key == null) {
            sqlStmt = "CALL InsertPerson (" +
                `'${jData.firstname}','${jData.middlename}','${jData.lastname}','${jData.email}',${prime},${dep},'${jData.affiliationid}','${jData.telephone}',
            '${jData.mobile}', ${isMinor});`;

        } else {
            sqlStmt = "CALL UpdatePerson (" +
                `${key},'${jData.firstname}','${jData.middlename}','${jData.lastname}','${jData.email}',${prime},${dep},'${jData.affiliationid}','${jData.telephone}',
                        '${jData.mobile}', ${isMinor});`;
    ...
    validateSession(uid, email, sessiontoken, aCallback) {
        const cell = "";
        var sqlStmt = "CALL authenticate(" + `'${uid}', '${email}', '${cell}', '${sessiontoken}',0);`;
        console.log(sqlStmt);
        executeStoreProc(sqlStmt, aCallback);
    }
    //
