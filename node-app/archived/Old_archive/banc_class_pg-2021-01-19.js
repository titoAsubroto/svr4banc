...
class setupDataHelper {
      ...
      if (hashedClientId in this.clientApiKeys) {
        ...
        if (apiKey == client.apikey) {
          ...
          if (api in this.api2AuthGroupMap) {
            ...
            out.group = group;
            out.valid = this.api2AuthGroupMap[api].includes(group);
          }
        }
      }
    }
  ...
  setLinkInfo() {
    //
    const sqlStmt =
      "SELECT type_name, type_id, display_name FROM banc.link_type;";
  ...
  setEventInfo() {
    const sqlStmt =
      "SELECT eventname, venue, entity_id, event_year, showlink, start_date, end_date FROM banc.event;";
    //
  ...
  getAccessInfo(req) {
    var out = {};
    out.token = (typeof req.header("stoken") === "undefined") ? "" : req.header("stoken");
    out.email = (typeof req.header("email") === "undefined") ? "" : req.header("email");
    out.uid = (typeof req.header("userid") === "undefined") ? "" : req.header("userid");
    out.apikey = (typeof req.header("apikey") === "undefined") ? "" : req.header("apikey");
    out.clientid = (typeof req.header("clientid") === "undefined") ? "" : req.header("clientid");
    out.formatType = (typeof req.header("formatType") === "undefined") ? "default" : req.header("formatType");
    out.fname = (typeof req.header("fname") === "undefined") ? "" : req.header("fname");
    out.mname = (typeof req.header("mname") === "undefined") ? "" : req.header("mname");
    out.lname = (typeof req.header("lname") === "undefined") ? "" : req.header("lname");
  ...
  isTokenValid(userid, email, token, actionCallback) {
    ...
    var sqlStmt =
      "SELECT primeid, personid, authlevel FROM banc.sessionToken WHERE uid=$1 and email=$2 and token=$3 " +
      expiryTime;
  ...
  setTransactionTypeInfo() {
    const sqlStmt =
      "SELECT category, subcategory, line_item, type_id, mapping_string, adult_count, child_count FROM banc.transaction_type;";
    this.pool.connect().then((client) => {
      return client
        .query(sqlStmt, null)
        .then((result) => {
  ...
  setAffiliationTypeInfo() {
    const sqlStmt =
      "SELECT affiliation, ismember, entity_id, mapping_string FROM banc.banc_community;";
    this.pool.connect().then((client) => {
      return client
        .query(sqlStmt, null)
        .then((result) => {
  ...
  getPersonSQL(jData, prime, dep, isMinor, key = null) {
    ...
    if (key <= 0 || key === null) {
      ...
      sqlStmt =
        "INSERT INTO banc.person (firstname, middlename, lastname, email, prime, " +
  ...
  setAssocLink(jData, key = null, aCallback) {
    ...
    if (key == null || key <= 0) {
      sqlStmt =
        "INSERT INTO banc.association_link" +
  ...
  getPersonWithPrime(
    ...
    if (primeid > 0 && personid > 0) {
      ...
      sql1 = "SELECT * from banc.person WHERE entity_id=$1;";
      sql2 =
        "SELECT * FROM banc.person WHERE entity_id=(SELECT entity1_id from banc.association_link " +
      ...
      sql1 =
        "SELECT * from banc.person WHERE (firstname='" +
        firstname +
        "' and lastname= '" +
        lastname +
        "' ";
  ...
  /*
   */
  collateMemberInfo(firstname, middlename, lastname, email, primeMemId, aCallback) {
    var resultSeq = 0;
    var primeId = 0;
    if (primeMemId > 0) {
      primeId = primeMemId;
    ...
    this.getPrimeMember(firstname, middlename, lastname, email, primeMemId, collateCB);
    //
