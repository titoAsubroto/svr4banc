...
const banc = require("./banc_class_pg-2021-01-19.js");
      ...
      if (flow_step == 0) {
        if (output.valid) {
          ...
          if (output.access_option == "user") {
            ...
            dHelper.getPersonWithPrime(null, null, null, accessInfo.email, primeId, personid, aPPersonCB);
    ...
    if (output.err === null) {
      if (output.valid) {
        if (output.access_option == "user") {
          ...
          dHelper.collateMemberInfo(null, null, null, null, primeId, aMembersCB);
        } else if (output.access_option == "client") {
          if (!isAuthorized(Res, calledServiceURL, output.authgroup)) {
            return;
          }
          /*
          dHelper.getPrimeMember(
            accessInfo.fname,
            accessInfo.mname,
            accessInfo.lname,
            accessInfo.email,
          ...
          dHelper.collateMemberInfo(accessInfo.fname,
            accessInfo.mname,
            accessInfo.lname,
            accessInfo.email, 0, aMembersCB);
        }
      } else {
        resultSeq = -1;
...
app.post("/banc/findPerson", (req, Res) => {
  ...
  if (strict) {
    sqlStmt =
      "SELECT * FROM banc.person WHERE firstname=$1 and lastname=$2 and middlename=$3 and email=$4;";
  } else {
    sqlStmt =
      "SELECT * FROM banc.person WHERE (firstname=$1 and lastname=$2 and middlename=$3) or (email=$4);";
  }
  ...
  function aFindPersonCB(output) {
    ...
    switch (flow_step) {
      ...
      case 0:
        ...
        if (prime == true) {
          sqlStmt =
            "Select entity2_id, entity1, entity2, name, link_type_id from banc.association_link where entity1_id=$1";
          dt = [entity_id];
      ...
      case 2:
        ...
        sqlStmt =
          "Select entity2_id, name, year, renewal_date, entity1, entity2 from banc.community_link where entity1_id=$1";
        dt = [outRes.primeid];
        flow_step = 3;
        execute1SqlsWithCommit(
          dHelper.setDbCallData("Find Person", sqlStmt, dt, null, null, false),
      ...
      case 3:
        ...
        sqlStmt =
          "Select al.name, e.eventname, e.event_year from banc.event e, banc.association_link al where " +
...
app.post("/banc/register", (req, Res) => {
  ...
  function aSetCredsCB(output) {
    ...
    if (flow_step == 0) {
      ...
      //
      var data_json = {
        sql1: "SELECT banc.setupcredential(" +
...
app.post("/banc/pay", (req, Res) => {
  ...
  if (!test.valid) {
    outRes = {
      msg: "Verifying banc transaction data. Inalid Pay Data submitted. " +
        test.msg,
      err: true,
  ...
  function aPaySubmitCB(output) {
    ...
    if (flow_step == 1) {
      ...
      let sql1 =
        "INSERT INTO banc.unprocessed_data (form_ref_id, banc_transaction_data, email) " +
