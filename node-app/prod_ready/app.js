...
app.post("/banc/processPayData", (req, Res) => {
  ...
  */
  ...
  var sql1 = "SELECT form_ref_id, banc_transaction_data, settlement_data, banc_data_processed, settlement_data_processed " +
  ...
  function aProcessDataCB(output) {
    ...
    outRes = {
      "msg": "Pay data retrived",
      "err": null,
      "raw_data": null,
      "settlement_data": null,
      "err_msg": null
    };
    if (sql1_result.rowCount == 1) {
      let  row = sql1_result.rows[0];
      let raw_data = JSON.parse(row["banc_transaction_data"]);
      let settlement_data = JSON.parse(row["settlement_data"]);
      outRes.raw_data = raw_data;
      outRes.settlement_data = settlement_data;
    }
