...
class OutputCovertor {
  constructor(verbose) {
    this.verbose = verbose ? true : false;
  }
  //Test function
  transform(jObject, fromType, toType) {
    ...
    for (const i in years) {
      ...
      if (year != "currentEvent" && year != "nextEvent") {
        for (const j in jObject.events[year]) {
          ...
          events[year] = jObject.events[year];
      }
    }
  ...
  convertGetMemberInfo(jObject, toType) {
    ...
    if (toType == "profile") {
      outJson.primaryMember = {
        name:
          jObject.prime.firstname +
          " " +
          jObject.prime.middlename +
          " " +
          jObject.prime.lastname,
        address:
          jObject.address.street +
          ", " +
          jObject.address.city +
          ", " +
          jObject.address.state +
          " - " +
          jObject.address.zip,
      ...
      outJson.spouse = {
        name:
          jObject.spouse.firstname +
          " " +
          jObject.spouse.middlename +
          " " +
          jObject.spouse.lastname,
...
exports.OutputConvertor = OutputCovertor;
