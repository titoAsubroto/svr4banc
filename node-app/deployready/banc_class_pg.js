...
class setupDataHelper {
    ...
    constructor(verbose) {
        ...
        this.eventInfo = {};
        this.assocLinkInfo = {};
        this.api2AuthGroupMap = this.config.api2AuthGroupMap;
        this.banc_org_id = "BANCinRDU";
        this.banc_entity_id = 100;
        this.org_entity = 'organization';
        this.result = null;
        this.insertId = null;
        this.authGroup = { "user": 1, "ec_member": 2, "ec_admin": 3 };
        this.db_config = {
    ...
    sortAndBuildTransactionData(form_ref_id, form_memo, form_date, jData) {
        ...
        */
        ...
        for (let i = 0; i < allTxns.length; i++) {
            ...
            if (txndata.category == "BANC") {
                ...
                if ("event" in jData) {
                    ...
                    out.form_memo = form_memo + "/" + txndata.memo + "/" + out.eventname + "/" + out.year;
                    smry.year = out.year;
                    let lineitem = txndata.line_item;
                    if (!smryEventText) {
                        smryEventText = " [" + out.eventname + " - " + out.year + "] " + lineitem + " ";
                    } else {
                        smryEventText = sumryEventText + ", " + lineitem;
                    }
        ...
        if (jData.cash.length > 0) {
            ...
            smry.form_memo = "Summary - " + smry.name;
    ...
    */
    ...
    executeAddressSQL(jAdr, primeid, key = null, aCallback) {
        ...
        if (key === null || key <= 0) {
            ...
            sqlStmt = ` "UPDATE ${this.entityTables["communication"]} SET street=${jAdr.street}, address2=${jAdr.address2}, ` +
    ...
    executeCommSQL(jFC, primeid, key = null, aCallback) {
        ...
        if (key === null || key <= 0) {
            ...
            sqlStmt = `INSERT INTO ${this.entityTables["communication"]} (telephone, mobile, email) VALUES (` +
                `'${jFC.telephone}','${jFC.mobile}','${jFC.email}') ON CONFLICT (street, city, zip) DO NOTHING RETURNING *;`;
            ...
            sqlStmt = ` "UPDATE ${this.entityTables["communication"]} SET email=${jFC.email}, telephone=${jFC.telephone}, ` +
                `mobile=${jFC.mobile} WHERE entit_id=${key}; `;
            sql2 = null;
            cond = false;
        }
        console.log(sqlStmt);
        this.execute2SqlTxn(this.setDbCallData("Execute Communication with prime", sqlStmt, null, sql2, null, cond), aCallback);
        return;
    }
    /* *************** */
    //
    ...
    */
    executePersonSQL(jData, primeid, prime, dep, isMinor, key = null, aCallback) {
        ...
        console.log(sqlStmt);
        this.execute2SqlTxn(this.setDbCallData("Execute Person with prime", sqlStmt, null, sql2, null, cond), aCallback);
        return;
    }
    //
    //
    ...
    setAssocLink(jData, key = null, aCallback) {
        ...
        this.execute1SqlTxn(this.setDbCallData("Set Assoc Link", sqlStmt, args, null, null, cond), aCallback);
        return;
    ...
    */
    ...
    processEventTransaction(primeid, jTxns, flowStep, anExtCallback) {
        ...
        //
        var flow_step = 0;
        var numOfEventTransactions = jTxns.len;
        var txnIter = 0;
    ...
    */
    processPersonsInfo(primeInfo, spouseInfo, depInfo, address, primcomm, loginInfo, aCallback) {
        ...
        //
        //aCallback(outRes);
        function personsCB(output) {
            var sql1_result = null;
            var sql2_result = null;
            let flow_step = outRes.flow_step;
            let update = outRes.loginInfo.update;
            //
            if (output != null) {
                if (output.err) {
                    outRes.err = output.err;
            ...
            if (flow_step == 0) {
                if (update) { // update prime if present
                    if (primeInfo != null) {
                        ...
                        executePersonSQL(primeInfo, primeid, prime, dep, isMinor, key, personsCB);
                        return;
                    }
                    flow_step = 1;
            ...
            if (flow_step == 1) {
                if (update) { // update spouse if present
                    if (spouseInfo != null) {
                        ...
                        outRes.flow_step = 2;
                        let primeid = primeInfo.entity_id;
                        executePersonSQL(spouseInfo, primeid, prime, dep, isMinor, key, personsCB);
                        return;
                    }
                    flow_step = 2;
            ...
            if (flow_step == 2) {
                if (update) { // update address if present
                    if (address != null) {
                        ...
                        outRes.flow_step = 3;
                        let primeid = outRes.prime.entity_id;
                        executeAddressSQL(address, primeid, key, personsCB);
                        return;
                    }
                    flow_step = 3;
                } else {  // insert if data is present
            ...
            if (flow_step == 3) {
                if (update) { // update primaryCommunication if present
                    if (primcomm != null) {
                        ...
                        outRes.flow_step = 4;
                        let primeid = outRes.prime.entity_id;
                        executeCommSQL(primcomm, primeid, key, personsCB);
                        return;
                    }
                    flow_step = 4;
                } else {  // insert if data is present
            ...
            if (flow_step == 5) {
                ...
                if (depLen > 0 && i < depsLen) {
                    if (update) { // update dep 
                        ...
                        outRes.sub_step = i + 1;
                        let primeid = primeInfo.entity_id;
                        executePersonSQL(aDep, primeid, prime, dep, isMinor, key, personsCB);
                        return;
                    } else {  // insert if data is present
                        let aDep = depInfo[i];
                        let key = null;
            ...
            if (flow_step == 6) {
                ...
                if (depLen > 0 && i < depsLen) {
                    if (!update) { // update dep 
                        ...
                        outRes.flow_step = 5;
                        outRes.sub_step = i + 1;
                        setAssocLink(jData, key, aCallback);
                        return;
                    }
                }
                flow_step = 10;
